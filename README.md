...
SEUPhyX 是东南大学物理实验中心的物理实验数据处理 Python 包。目前仓库中的主要应用是「密立根油滴实验」人工智能辅助数据处理系统，基于 Streamlit 提供网页交互界面，用于学生实验数据记录、机器学习 Q 聚类、符号回归、传统方法对照和实验报告生成。
...
传统机器学习分类页仍保留在「传统方法对照」中，主要用于教学对照、个人模型训练和可视化；其 `Predicted` 标签不是实验真值，也不是当前 AI 聚类与符号回归的必要输入。`regression.py` 文件末尾保留的 `physics_guided_regression()` 是旧版整数 `n` 物理约束拟合路径，可作为兼容或对照逻辑阅读。
...
5. 在「机器学习—符号回归」页选择参与回归的簇，对多个 `q` 簇的 `U-t` 曲线做共享符号表达式搜索，并可展开传统物理拟合作为对照检验。
6. 可在「传统方法对照」页查看或训练旧版 `U-t` 散点分类模型。该步骤是对照流程，不是 AI 聚类与符号回归的前置条件。
...
- 报告生成依赖已经完成机器学习—符号回归；若没有 `data_pred`，报告页会退回使用 `regression_results["clusters"]`。
