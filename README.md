...
3. 由 `U` 和 `t` 换算每个油滴的电荷估计 `q`。
4. 使用 KDE 峰识别、半峰宽筛选和共享符号表达式搜索，在不预设元电荷和整数倍关系的情况下发现 `q` 的聚类结构与曲线通式。
5. 生成包含学生信息、q 峰发现、共享拟合结果的 PDF 实验报告。
...
- 在 `q` 分布上进行 KDE 峰识别，不预设元电荷大小或整数电荷数。
- 使用半峰宽筛选高置信数据点，保留低置信点作为可视化对照。
- 对发现的多个 q 峰曲线进行共享符号表达式搜索，自动归纳类似 `U(t,Q_c)=b+A/(Q_c*t^(3/2))` 的通式。
