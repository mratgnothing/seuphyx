                            ...
                            with btn_cols[i * 2 + 1]:
                                if st.button("✕", key=f"del_raw_{int(did)}_{meas_no}",
                                    ...
                                    if not remaining.empty:
                                        ...
                                        new_u = (
                                            sum(remaining["BalanceVoltage(U/V)"].tolist())
                                            / len(remaining)
                                        )
                                        if "AvgTime(t/s)" in df_raw_new.columns:
                                            df_raw_new.loc[
                                                remaining.index,
                                                "AvgTime(t/s)",
                                            ] = round(new_avg, 3)
                                        if "AvgVoltage(U/V)" in df_raw_new.columns:
                                            df_raw_new.loc[
                                                remaining.index,
                                                "AvgVoltage(U/V)",
                                            ] = round(new_u, 1)
                                        ...
                                        if main_csv.exists():
                                            ...
                                            if mask.any():
                                                ...
                                                df_main.loc[mask, "BalanceVoltage(U/V)"] = round(new_u, 1)
